error id: file://<WORKSPACE>/flight_prediction/src/main/scala/es/upm/dit/ging/predictor/MakePrediction.scala:format.
...
non-local guesses:
	 -com/mongodb/spark/finalPredictions/writeStream/format.
	 -com/mongodb/spark/finalPredictions/writeStream/format#
	 -com/mongodb/spark/finalPredictions/writeStream/format().
	 -spark/implicits/finalPredictions/writeStream/format.
	 -spark/implicits/finalPredictions/writeStream/format#
	 -spark/implicits/finalPredictions/writeStream/format().
	 -finalPredictions/writeStream/format.
	 -finalPredictions/writeStream/format#
	 -finalPredictions/writeStream/format().
	 -scala/Predef.finalPredictions.writeStream.format.
	 -scala/Predef.finalPredictions.writeStream.format#
	 -scala/Predef.finalPredictions.writeStream.format().
offset: 6309
...
object MakePrediction {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val predictionsAsJson = finalPredictions.select(
      to_json(struct(finalPredictions.columns.map(col): _*)).alias("value")
    )

    // Write predictions to Kafka topic
    val kafkaStreamWriter = predictionsAsJson
      .writeStream
      .format("kafka")
      .option("kafka.bootstrap.servers", "localhost:9092")
      .option("topic", "flight-delay-ml-predictions")
      .option("checkpointLocation", "/tmp/chk-flight-predictions-kafka")
      .outputMode("append")

    val kafkaQuery = kafkaStreamWriter.start()

    // Also write to MongoDB for persistence
